...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
  if(isset($_POST["submit_mail"]))
  {
    if(isset($_POST['reset-username']) && isset($_POST['reset-mail']))
    {
                ...
                $result1 = mysqli_query($link,"SELECT * FROM farms WHERE email ='$mail' AND username='$username'");
                 ...
                 if(mysqli_num_rows($result1) > 0)
                 {
                              ...
                              if($mail_status == 1)
                              {
                                  ...
                                  if($result)
                                  {
                                      ...
                                      $_SESSION["success_user"] = $username;
                                  }
                              }
                              // else {
  ...
  if(isset($_POST["submit_pass"]))
  {
    if(isset($_POST['reset-password']) && isset($_POST['reset-confirm']) )
    {
                ...
                $reset_user = $_SESSION["success_user"];
                $usermail = $_SESSION["usermail"];
                if($password == $confirm_password)
                {
                  $pass = password_hash($password,PASSWORD_DEFAULT);
                  $result = mysqli_query($link,"UPDATE farms SET password = '$pass' WHERE username = '$reset_user'");
                  if($result)
                  {
                    $response["success"] = 1;
                    $response["message"] = "Password Changed Successfully";
                    require_once("mail_function_admin.php");
                    $mail_status = sendFarmUpdateNotif($usermail,$reset_user,$password);
                    $success = "";
                    $error_message = "";
                    $success_user = "";
                    $_SESSION = array();
                    ...
                    echo 'window.location.href = "farmLogin.php";';
                    echo '</script>';
                  }
...
<head>
    <meta charset="UTF-8">
    <title>EEKI Farm User Reset</title>
...
<body>
    <div class="signup-form">
        <h2>EEKI Farm User password Reset</h2>
        <p></p>
        <!-- <form action="" method="post"> -->
          ...
          <?php
            ...
            <div class="form-group">
                ...
                <input type="text" name="reset-username" class="form-control" placeholder="Farm User" required>
            </div>
            <div class="form-group">
                <label>Email Id</label>
                <input type="text" name="reset-mail" class="form-control" placeholder="Farm user mail id" required>
            </div>
            <div class="form-group">
