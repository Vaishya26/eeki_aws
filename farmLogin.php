...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($username_err) && empty($password_err)){
        ...
        $sql = "SELECT id, username, password FROM farms WHERE username = ?";
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    ...
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($password, $hashed_password)){
                            ...
                            $_SESSION["farm_id"] = $id;
                            $_SESSION["farm_username"] = $username;
                            $ip = mysqli_query($link,"SELECT * FROM farms WHERE username ='$username'")or die(mysql_error());
                            $row = mysqli_fetch_assoc($ip);
                            // Redirect user to master page
                            header("location: http://".$row['farmIp'].":".$row['farmPort']."/eekiBoard?farm_id=".$row['farmId']."&farm_name=".$row['farmName']."&farm_user=".$row['username']."&farm_userType=farmuser");
                           } else{
...
<head>
	<title>EEKI Foods Farm Owner Login</title>
...
The objective is achieved by setting up farms with a farm-partner across on outskirts of major urban areas of India. These farms produce high-quality residue-free fruits and vegetables using our IOT enabled completely medium-less growing technology. “We are not consultants. We are growers, the technology enablers.”</p>
				...
				<div class="form-left">
					<div class="middle">
						<h4>Farm Login</h4>
						<p>Login here to Configure/Monitor Farm</p>
					</div>
              ...
              <span style="float:right;"><a href="reset.php">Forgot Password?</a></span>
					</form>
